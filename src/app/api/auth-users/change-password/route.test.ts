...
describe("definição de senha por capability", () => {
  ...
  it("aceita qualquer senha com ao menos oito caracteres, sem regra de composição", async () => { maybeSingle.mockResolvedValue({ data: { must_change_password: true } }); verifyPurpose.mockReturnValue(null); expect((await POST(request("", "abcdefgh"))).status).toBe(200); expect(updateUser).toHaveBeenCalledWith({ password: "abcdefgh" }); });
  it("rejeita senha com sete caracteres", async () => { maybeSingle.mockResolvedValue({ data: { must_change_password: true } }); verifyPurpose.mockReturnValue(null); expect((await POST(request("", "abcdefg"))).status).toBe(400); expect(updateUser).not.toHaveBeenCalled(); });
...
function request(cookie = "", newPassword = "Synthetic-Strong-Password-42!") { return new Request("https://app.invalid/api/auth-users/change-password", { method: "POST", headers: { origin: "https://app.invalid", "content-type": "application/json", cookie }, body: JSON.stringify({ newPassword }) }); }
